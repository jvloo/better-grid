...
- **npm publish credentials live in `.credentials/.npmrc`** (gitignored — see `.gitignore` entry `.credentials/`). When the user asks you to publish to npm, source the token from there: `cp .credentials/.npmrc ./.npmrc && pnpm -r publish --access public --no-git-checks && rm ./.npmrc` (the temporary copy at the repo root is read by pnpm; remove it as soon as publish completes so it never gets staged). If `.credentials/.npmrc` is missing, stop and ask the user — never invent a token, never use a token from another repo, never disable 2FA, never publish without verifying the token works.
