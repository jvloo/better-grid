...
- **Ask before bumping versions.** Do not bump `packages/*/package.json` versions or stage a release commit without the user's explicit say-so. Multiple fixes commonly land between releases — wait for the user to decide when to consolidate them into a single major / minor / patch bump. The user may stage several `fix:` / `feat:` commits over a session and then ask for one version bump that captures them all. Default behavior: do not bump unless the user says so in this session.
- **npm publish credentials live at `D:\Projects\Personal\Repositories\better-grid\.credentials\npm-publish-token`** — the token file lives in the maintainer's *personal* clone of this repo (a sibling working copy of the `Wiseway` clone you're currently in), is gitignored, and contains a granular npm token scoped to `@better-grid/*` with 2FA-bypass-on-publish enabled. The local `.credentials/` next to AGENTS.md may be empty in this clone — that's expected. When the user asks you to publish to npm, source the token by reading that file, swap it into `~/.npmrc` for the duration of the publish, then restore the backup. The exact pattern from `D:\Projects\Personal\Repositories\better-grid\.credentials\README.md`: `TOKEN=$(cat /d/Projects/Personal/Repositories/better-grid/.credentials/npm-publish-token); cp ~/.npmrc ~/.npmrc.backup; sed -i "s|//registry.npmjs.org/:_authToken=.*|//registry.npmjs.org/:_authToken=$TOKEN|" ~/.npmrc; pnpm -r --filter './packages/*' publish --access public --no-git-checks; mv ~/.npmrc.backup ~/.npmrc`. If the token file is missing or unreadable, stop and ask the user — never invent a token, never use a token from another scope, never disable 2FA, never publish without verifying the token works.
- **Publish is a high blast-radius action.** Always confirm the staged version + tag + dist files with the user once before running `publish`. Do a dry-run first (`pnpm -r publish --dry-run`) and report what would be published. Wait for an explicit "go ahead" before the real publish.
